...
static void	render_format(t_data *data, char specifier)
{
	...
	int_val = 0;
	if (specifier == '%')
		data->chars_written = print_char(data, '%');
	else if (specifier == 'c')
		data->chars_written = print_char(data, va_arg(data->ap, int));
	else if (specifier == 's')
		data->chars_written = print_str(data, va_arg(data->ap, char *));
	if (inside("dioupxXu", specifier))
	{
		...
		data->chars_written = print_digit(data, int_val);
	}
}
...
int	partial_init(t_data *data)
{
	...
	data->flags.star = false;
	data->flags.left_just = false;
	data->flags.hash = false;
